...
// POST /guard/status - Read-only autonomous Guard state for a wallet (for MCP + web).
app.post("/guard/status", async (c) => {
  const { walletAddress } = walletSchema.parse(await c.req.json());
  const wallet = normalizeAddress(walletAddress);

  const guardConfig = await supabaseService.getGuardConfig(wallet).catch(() => null);
  const baselinePrices = await supabaseService.getBaselinePrices(wallet).catch(() => ({}));
  const raw = await supabaseService.getHistory(wallet, 5).catch(() => [] as any[]);
  const recentActivity = raw.map((event: any) => ({
    type:
      event.event_type === "rebalance"
        ? "autonomous_save"
        : event.event_type === "manual_rebalance"
        ? "fix"
        : event.event_type === "guard_armed"
        ? "guard_armed"
        : "diagnosis",
    timestamp: event.created_at,
    summary:
  ...
  return c.json({
    ...
    watching: watcher.isArmed(wallet),
    clusterToken: watcher.clusterTokenFor(wallet) ?? null,
    baselinePrices,
    recentActivity,
    webLink: `/guard/${wallet}`,
  });
});
